...
| `isFilterable`     | `is-filterable`     | Whether the options list should be filtered by the text input                                                                                                                           | `boolean` | `false`        |
| `isLookup`         | `is-lookup`         | Use the Lookup style (i.e. swap the caret for a search icon)                                                                                                                            | `boolean` | `false`        |
...
| `selectionType`    | `selection-type`    | Whether/how changes to the text field value select an option from the list. Possible values are "manual" (default) and "automatic". Any value other than "automatic" will equal manual. | `string`  | `"manual"`     |
