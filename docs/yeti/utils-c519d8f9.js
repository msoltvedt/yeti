...
const utils = {
  ...
  getType: function (dunno) {
    if (typeof dunno != "string") {
      console.error('getType assumed string, but got', typeof dunno);
    }
    dunno = dunno.toString();
    if (utils.isConvertableToDate(dunno)) {
      console.log(`${dunno} is date`);
      return "date";
    }
    else {
      if (utils.isConvertableToNumber(dunno)) {
  ...
  castToNumber: function (someString) {
    ...
    console.log('in castToNumber, have de-comma-ified, someString was', someString, typeof someString);
