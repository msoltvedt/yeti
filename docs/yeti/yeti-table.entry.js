...
const YetiTable = class {
    ...
    renderCell(cell, containingRow, isRowExpanderCell = false) {
        if (!cell.id || cell.id == "") {
        ...
        if (cell.isHeading) {
            ...
            return h("td", Object.assign({ class: 'yeti-table-cell' + css, id: cell.id, key: cell.id }, ((cell.template) ? { "innerHTML": cell.template } : {}), ((cell.colspan && typeof cell.colspan == "number") ? { "colspan": cell.colspan } : {}), ((cell.rowspan && typeof cell.rowspan == "number") ? { "rowspan": cell.rowspan } : {})), (isRowExpanderCell ? /* If this is an expandable row...*/
    ...
    renderRows(rowStartIndex = 0, rowsToDisplay = this.contents.body.rows.length) {
        ...
        if (this.contents.body.rows.length == 0 || !this.contents.body.rows[0].cells || this.contents.body.rows[0].cells.length == 0) {
            ...
            return h("tr", { class: "yeti-table-body-row" }, h("td", { class: "yeti-table-cell", colSpan: colspan }, this.placeholderText));
        }
                ...
                if ((i >= rowStartIndex) && // This row is on or after the startIndex,...
                    ...
                    if (row.childRows && row.childRows.length && row.childRows.length > 0) {
                        ...
                        for (let c = 0; c < row.childRows.length; c++) {
                            tbodyContents.push(h("tr", { class: childRowCSS, id: `${row.id}_child_${c}`, key: `${row.id}_child_${c}` }, this.renderRow(row.childRows[c])));
                        }
                    }
                }
            }
